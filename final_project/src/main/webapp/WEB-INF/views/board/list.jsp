...
<c:choose>
	...
	<div class="container">
		...
		<%-- 이전 버튼 --%>
		<c:if test="${artiNum>9}">
			<a href="list?b_category=${param.b_category}&b_article=${param.b_article}&next=${next-1}&artiNum=${(next-1)*10+9}">&lt; 이전</a>
		</c:if>
		<%--게시판 15개 나눈 모음(?) 번호 --%>
		<c:choose>
			<c:when test="${count > next*10+10 }">
				<c:forEach begin="${next*10+1}" end="${next*10+10}" step="1" var="cnt">
					<a href="list?b_category=${param.b_category}&b_article=${param.b_article}&next=${next}&artiNum=${cnt-1}">[${cnt}]</a>
				</c:forEach>
			</c:when>
			<c:otherwise>
